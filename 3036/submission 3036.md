...
```text
โจทย์ให้ฉันรับเลขห้องที่อยู่ และหาว่าจะเดินทางไปยังห้องหมายเลข 1 ต้องพังกำแพงน้อยที่สุดกี่กำแพง

Input:
โปรแกรมรับจำนวนเต็มหนึ่งจำนวนN

Output:
โปรแกรมควรพิมพ์จำนวนกำแพงที่น้อยที่สุดที่ต้องพังเป็นจำนวนเต็ม หากไม่ต้องพังให้แสดง "0"
...
```text
Step 1:รับหมายเลขห้องเป็นจำนวนเต็ม N
Step 2:กำหนดค่า row = 1 เพื่อใช้เก็บหมายเลขแถว
Step 3:ตรวจสอบว่า row**2 น้อยกว่า N หรือไม่
Step 4:ถ้า row**2 น้อยกว่า N ให้เพิ่มค่า row ขึ้นทีละ 1
Step 5:ทำซ้ำ Step 3-4 จนกว่า row**2 จะมากกว่าหรือเท่ากับ N
Step 6:คำนวณตำแหน่งของห้อง N ในแถวนั้น โดย position = N - (row - 1)**2
Step 7:ตรวจว่า position เป็นเลขคี่หรือคู่ แล้วคำนวณตามเงื่อนไขของโจทย์
Step 8:พิมพ์จำนวนห้องที่น้อยที่สุด
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับวิธีแรก ฉันใช้ modulo เพื่อตรวจว่าเป็นเลขคี่หรือคู่ จากนั้นหาค่า row และ position เพื่อนำไปคำนวณคำตอบ โดยถ้า position เป็นเลขคี่จะคำนวณแบบหนึ่ง ถ้าเป็นเลขคู่จะคำนวณอีกแบบหนึ่ง
```
...
```text
ตรวจค่า N ถ้ารับมาเป็นห้องหมายเลข 1 อยู่แล้วจะคำนวณจำนวนห้องหรือไม่
```
...
```text
ตรวจกรณีที่ position เป็นเลขคี่
```
...
```text
ตรวจกรณีที่ position เป็นเลขคู่
```
...
```text
เงื่อนไขของโค้ดในส่วนของการเพิ่ม row 
```

คุณยังทำอะไรด้วยตนเอง

```text
ฉันเขียนแผน เขียน code บางส่วน ทดสอบ และส่ง OJ ด้วยตนเอง
```
