...
```text
โจทย์ให้ฉันคำนวณค่าธรรมเนียมการส่งพัสดุ

Input:
โปรแกรมรับค่าต้นทาง ปลายทาง เป็นภาษาอังกฤษ และรับค่าน้ำหนักพัสดุ

Output:
โปรแกรมควรแสดงผลราคาการนำส่งพัสดุ หากเส้นทางที่ระบุไม่มีในที่กำหนดโปรแกรมควรพิมพ์ "Error"

Constraints:
input รับค่าต้นทาง ปลายทาง ต้องอยู่บรรทัดเดียวกัน และ ค่าน้ำหนักพัสดุต้องเป็นจำนวนจริง
output ต้องแสดงทศนิยมสองตำแหน่ง
```
...
```text
STEP 1:รับค่าต้นทาง ปลายทางโดยแปลงเป็นตัวอักษรพิมพ์ใหญ่
STEP 2:รับค่าน้ำหนักพัสดุ
STEP 3:ตรวจสอบค่าต้นทาง ปลายทาง ว่าตรงกับเงื่อนไขที่กำหนดหรือไม่
STEP 4:หลังจากตรวจสอบค่าต้นทาง ปลายทางแล้วเลือกสูตรคำนวณตามที่กำหนดแต่ละเส้นทาง
STEP 5:นำค่าน้ำหนักพัสดุไปคำนวณรวมกับค่าธรรมเนียมเริ่มต้นและอัตราขนส่งของเส้นทางนั้น
STEP 6:แสดงผลราคา โดยกำหนดให้แสดงทศนิยมสองตำแหน่ง
STEP 7:หากต้นทาง ปลายทางไม่ตรงกับเงื่อนไขที่กำหนด ให้พิมพ์"Error"
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันใช้ if และ elif เพื่อตรวจสอบเงื่อนไขของเส้นทาง จากนั้นคำนวณค่าขนส่งโดยนำค่าธรรมเนียมเริ่มต้นบวกด้วยค่าน้ำหนักที่คูณกับอัตราค่าขนส่งของแต่ละเส้นทาง จากนั้นพิมพ์ผลราคาขนส่งออกมา หากเส้นทางที่ไปไม่ตรงกับเงื่อนไขจะพิมพ์ "Error"
```
...
```text
ตรวจ กรณีที่เส้นทางถูกต้องตรงตามเงื่อนไข
```
...
```text
ตรวจ edge case เมื่อน้ำหนักพัสดุเป็น 0 
```
...
```text
ตรวจ กรณีเส้นทางไม่ตรงกับเงื่อนไขที่กำหนด
```
...
```text
ช่วยคิดสมการ และ เขียน code บางส่วน
```

คุณยังทำอะไรด้วยตนเอง

```text
คิดสมการ และ เขียน code บางส่วน
```
