...
```text
โจทย์ให้ฉันรับค่าจำนวนนับA,B,d และ r แล้วหาว่าจำนวนนับที่อยู่ระหว่าง A ถึง B มีกี่จำนวนที่หารด้วยจำนวนนับ d แล้วเหลือเศษ r

Input:
โปรแกรมรับจำนวนนับ A,B,d และ r

Output:
จำนวนของจำนวนนับระหว่าง A ถึง B ที่หารด้วย d แล้วเหลือเศษ r
...
```text
Step 1:รับค่าจำนวนนับ A,B,d และ r
Step 2:ตั้งตัวแปร count เพื่อใช้เก็บจำนวนตัวเลขที่ตรงเงื่อนไข
Step 3:ใช้ for loop เพื่อตรวจจำนวนนับตั้งแต่ A ถึง B
Step 4:ตรวจว่าตัวเลขที่อยู่ระหว่าง A ถึง B ว่ามีตัวไหนที่หารด้วย d แล้วเหลือเศษเท่ากับ r
Step 5:ถ้าตรงตามเงื่อนไข ให้บวก count ขึ้นทีละ 1
Step 6:พิมพ์จำนวนตัวเลขที่อยู่ระหว่าง A ถึง B ที่ตรงตามเงื่อนไข
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันรับค่าตัวแปร A, B, d แล้วก็ r เข้ามา แล้วตั้งตัวแปร count เป็น 0 ไว้รอเก็บจำนวน จากนั้นใช้ for loop ตรวจดูเลขตั้งแต่ A ถึง B ทีละตัว แล้วเอาเลขแต่ละตัวมาหาร d ด้วย modulo แล้วตรวจว่าเหลือเศษเท่ากับ r หรือไม่ ถ้าใช่ก็บวก count เพิ่มทีละ 1 แล้วสุดท้ายก็พิมพ์ค่า count ออกมา
```
...
```text
ตรวจว่าช่วงตัวเลขจาก 10 ถึง 20 ที่หารด้วย 3 แล้วเหลือเศษ 1 
```
...
```text
ตรวจกรณีที่เริ่มต้นและสิ้นสุดเป็นตัวเดียวกันก็คือ 5 หารด้วย 2 แล้วเหลือเศษ 1 
```
...
```text
ตรวจกรณีที่ตัวหารมีค่ามากกว่าตัวเลขบางตัวในช่วง ให้ช่วง 1 ถึง 10 ที่หารด้วย 5 แล้วเหลือเศษ 4 
```
