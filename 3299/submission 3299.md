...
```text
โจทย์ให้ฉันหาว่าถ้าปลูกดอกไม้ไปทั้งหมด N ช่อง โดยปลูกเรียงเป็นแนวทแยงและแต่ละแถบมีความหนา L ช่อง แล้วหาว่าช่องสุดท้ายที่ปลูกจะอยู่ในแถบที่เท่าไหร่

Input:
โปรแกรมรับจำนวนเต็ม 2 จำนวนคือ L (ความหนาของแถบ) และ N (จำนวนช่องที่ปลูก) ที่คั่นด้วยช่องว่าง เป็นแบบบรรทัดเดียว

Output:
โปรแกรมควรแสดงผลลัพธ์หมายเลขแถบช่องสุดท้าย เป็นจำนวนเต็มหนึ่งตัว
...
```text
Step 1: รับค่า l (ความหนาของแถบ) และ n (จำนวนช่องที่ปลูก) จากบรรทัดเดียว
Step 2: ตั้งตัวนับแนวทแยง d เริ่มที่ 0 (d คือจำนวนแนวทแยงที่ปลูกเต็มไปแล้ว)
Step 3: แนวทแยงที่เส้น k มี k ช่อง ดังนั้นแนวทแยงที่ 1 ถึง d รวมกันจะมี d * (d+1) / 2 ช่อง วนเพิ่ม d ทีละ 1 ถ้าผลรวมยังมีค่าน้อยกว่า n อยู่
Step 4: วนจบ d จะเป็นหมายเลขแนวทแยงที่ n ตกอยู่
Step 5: ทุกๆ l แนวทแยงเป็น 1 แถบ หมายเลขแถบจะเท่ากับ (d+l-1) // l ซึ่งคือ d 
Step 6: พิมพ์ผลลัพธ์หมายเลขแถบที่ได้
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันรับค่า l กับ n แล้วให้ d เริ่มที่ 0 จากนั้นวนเพิ่ม d ทีละ 1 ตราบใดที่ผลรวมช่องของแนวทแยงที่ 1 ถึง d ที่คำนวณจากสูตร (d*(d+1)//2) ยังน้อยกว่า n อยู่ เมื่อวนจบ d ก็คือหมายเลขแนวทแยงที่ช่องที่ n อยู่ แล้วจะแปลงเป็นหมายเลขแถบด้วยสูตร (d+l-1)//l จากนั้นพิมพ์หมายเลขแถบนั้น
```
...
```text
ตรวจกรณีที่ค่า n น้อยที่สุดแต่ l ใหญที่สุด
```
...
```text
ตรวจกรณีที่ n ตรงกับช่องสุดท้ายของแนวทแยงพอดี
```
...
```text
ตรวจกรณีที่เป็นค่าสูงสุดของโจทย์ (ขอบบน)
```
...
```text
ช่วยคิดแผนบางส่วน และ ตรวจ code
```

คุณยังทำอะไรด้วยตนเอง

```text
ฉันเขียนแผนบางส่วน เขียน code ทดสอบ ด้วยตนเองทั้งหมด
```
