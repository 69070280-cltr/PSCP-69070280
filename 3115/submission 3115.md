...
```text
โจทย์ให้ฉันรับช่วงเวลาที่ร้านเปิดให้บริการ โดยแต่ละช่วงจะมีเวลาเริ่มต้นและเวลาสิ้นสุด แล้วรับเวลาที่ต้องการตรวจสอบหลายเวลา ตรวจสอบแต่ละเวลาโดยต้องนับว่ามีช่วงเวลาของร้านกี่ช่วงที่ครอบคลุมเวลานั้น แล้วแสดงจำนวนช่วงที่ครอบคลุมของแต่ละเวลาตามลำดับ

Input:
โปรแกรมรับจำนวนช่วงเวลาที่ต้องตรวจสอบ
โปรแกรมรับเวลาเริ่มต้นและเวลาสิ้นสุดของแต่ละช่วง
โปรแกรมรับเวลาที่ต้องการตรวจสอบ

Output:
โปรแกรมควรแสดงจำนวนช่วงเวลาที่ครอบคลุมเวลาที่ตรวจสอบแต่ละเวลา

Constraints:
เวลาที่กำหนดต้องอยู่ในช่วง 0 ถึง 1440 นาที และต้องตรวจสอบว่าเวลาที่กำหนดอยู่ในช่วงเริ่มต้นและสิ้นสุดหรือไม่
```
...
```text
Step 1:รับจำนวนช่วงเวลา แล้วรับเวลาเริ่มกับเวลาจบของแต่ละช่วงมาเก็บไว้
Step 2:รับเวลามาตรวจสอบ
Step 3:นำเวลาที่ต้องการตรวจสอบแต่ละตัว
Step 4:ตรวจสอบว่าเวลานั้นอยู๋ในช่วงเริ่มต้นถึงสิ้นสุดหรือไม่ถ้าอยู่ก็ให้บวกเพิ่มทีละ 1
Step 5:ตรวจสอบให้ครบทุกช่วง แล้วเก็บจำนวนช่วงที่นับได้ของแต่ละเวลา
Step 6:แสดงผลจำนวนช่วงที่เวลานั้นอยู๋ในช่วง
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก โดยเริ่มจากเก็บค่าเวลาเริ่มต้นและสิ้นสุดของแต่ละช่วงไว้ก่อน แล้วนำเวลาที่ได้มาตรวจทีละค่า จากนั้นตรวจสอบว่าเวลานั้นอยู่ใสช่วงที่กำหนดไว้หรือไม่ ถ้าอยู่ก็บวกเพิ่มจำนวนทีละ 1 แล้วตรวจสอบไปจนครบทุกช่วง แล้วแสดงจำนวนนับที่ได้ของแต่ละช่วง
```
...
```text
ตรวจกรณีที่เวลายังอยู๋ในช่วงที่กำหนด
```
...
```text
ตรวจกรณีที่มีช่วงเวลาที่ซ้อนกัน
```
...
```text
ช่วยเขียน code บางส่วน และช่วยตรวจการวางแผนที่คิดไว้แต่ละขั้นตอนว่าสามารถทำได้หรือไม่
```

คุณยังทำอะไรด้วยตนเอง

```text
คิดแผนและเขียน code เองบางส่วน
```
