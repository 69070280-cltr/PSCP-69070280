...
```text
โจทย์ให้ฉันหาจำนวนเฉพาะ พร้อมระบุจำนวน

Input:
โปรแกรมรับค่าจำนวนเริ่มต้นและจำนวนสุดท้าย

Output:
โปรแกรมแสดงจำนวนเฉพาะ และ ระบุจำนวนของจำนวนเฉพาะที่อยู่ในช่วง

Constraints:
input รับค่าแบบบรรทัดเดียว
```
...
```text
Step 1:รับค่าจำนวนเต็ม n (แทนจำนวนเริ่มต้น) และ t (แทนจำนวนสุดท้าย)
Step 2:ตรวจจำนวนตั้งแต่ n ถึง t ทีละจำนวน
Step 3:ตรวจว่าแต่ละจำนวนเป็นจำนวนเฉพาะหรือไม่
Step 4:ถ้าเป็นจำนวนเฉพาะให้เก็บใส่ list
Step 5:พิมพ์ list จำนวนเฉพาะ และจำนวนทั้งหมดที่เจอในช่วง n ถึง t
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันใช้ for loop เพื่อวนหาจำนวนเฉพาะตั้งแต่จำนวนที่ n ถึงจำนวนที่ t โดยตรวจว่าแต่ละตัวหารด้วยจำนวนตั้งแต่ 2 ถึงตัวเลขก่อนหน้าตัวมันเองว่าหารลงตัวหรือไม่ ถ้าหารลงตัวแสดงว่าไม่ใช่จำนวนเฉพาะแต่ถ้าไม่มีตัวไหนหารลงตัวแสดงว่าเป็นจำนวนเฉพาะและเก็บไว้ใน list และก่อนตรวจตัวหารจะตรวจก่อนว่าตัวเลขน้อยกว่า 2 หรือไม่ ถ้าน้อยกว่าจะข้ามไปเพราะ 0 และเลขติดลบไม่ใช่จำนวนเฉพาะ จากนั้นแสดงตัวเลขทั้งหมดที่เป็นจำนวนเฉพาะและจำนวนเฉพาะของจำนวนในช่วง n ถึง t
```
...
```text
ตรวจกรณี edge case ที่ค่าเริ่มต้นและสิ้นสุดเป็นตัวเดียวกัน
```
...
```text
ตรวจกรณีช่วงที่กำหนดไม่มีจำนวนเฉพาะ
```
...
```text
ตรวจกรณี edge case ค่าเริ่มต้นเป็นเลขติดลบ เพื่อทดสอบเงื่อนไขที่จะไม่นำเลขติดลบและเลข 0 มาหาจำนวนเฉพาะ
```
